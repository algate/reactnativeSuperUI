...
export default class example extends Component {
    ...
    scrollInterpolator = (index, props) => {
      ...
      const inputRange = getInputRangeFromIndexes(outputRange, index, props);
      return { inputRange, outputRange };
    };
  
    slideInterpolatedStyle = (index, animatedValue, props) => {
      const card1Scale = 0.85;
      const card2Scale = 0.7;
  
      const { data, itemWidth } = props;
      const getTranslateFromScale = (cardIndex, scale) => {
        const centeredPosition = -Math.round(itemWidth * cardIndex / scale);
        const edgeAlignment = Math.round((itemWidth * (1 - scale)) / 2 / scale);
        const offset = Math.round(OFFSET * cardIndex / scale);
        const drift = -Math.round(OFFSET / scale);
        return centeredPosition + edgeAlignment + offset + drift;
      };
    ...
    layoutExampleCustom (number, title, type) {
        const isTinder = type === 'tinder';
        return (
            <View style={[styles.exampleContainer, isTinder ? styles.exampleContainerDark : styles.exampleContainerLight]}>
                <Text style={[styles.title, isTinder ? {} : styles.titleDark]}>{`Example ${number}`}</Text>
